...
| ---------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------- | ---------------------------------------------------- |
| `aerpAuthorize`              | `36_AuthorizationEngine.js`; `AERP-036_AuthorizationEngine.js` | Incompatible contracts: `(request, options)` versus `(context, model)` | `incompatible-public-entrypoint`, CRITICAL; blocking |
| `AERP_AUTHORIZATION_VERSION` | `36_AuthorizationEngine.js`; `AERP-036_AuthorizationEngine.js` | Apparently equivalent value (`1.0.0`), but duplicate global ownership  | `equivalent-duplicate`, HIGH; blocking               |
| `aerpMetadataToRow`          | `04_ActionEngine.js`; `07_Writer.js`                           | Apparently equivalent signature and implementation                     | `equivalent-duplicate`, HIGH; blocking               |
...
The Full Suite Gate and the Globals Gate are implemented and active. Their implementations and executions have been approved, but the repository currently fails the Globals Gate. Every other row remains a future gate with status Future / Pending.
...
| Globals Gate             | Implemented / Active | Operational: `npm run qa:globals`; self-tests: `npm run test:gate:globals` | Closed inventory: 38 included, 7 excluded, 45 discovered; parser-backed global declarations, assignments, and dynamic constructions | Valid execution currently reports 758 unique symbols, 781 occurrences, 3 duplicate symbols, 7 blocking dynamic constructions, and exit `1 / STRUCTURAL_BLOCKERS` | Architecture owner | Implementation/integrity failure, inventory mismatch, parse gap, unresolved collision, or blocked dynamic construction; current repository result is blocked | Current local execution evidence: Git-ignored `.qa-output/globals-evidence.json`, schema `1.1.0`; ephemeral, revision-bound and `runId`-bound; regenerated on every run; not reusable for another review |
| Globals release evidence | Future / Pending     | External retention from clean checkout or CI                               | Exact revision, inventory, command, result, and evidence hash                                                                       | Versioned or immutable release record                                                                                                                            | Release owner      | Missing, mutable, reused, or revision-mismatched evidence                                                                                                    | Future release evidence; current local JSON does not satisfy this row                                                                                                                                    |
...
The Full Suite Gate's `.qa-output/test-evidence.json` and the Globals Gate's `.qa-output/globals-evidence.json` are local execution evidence, not release evidence. Both are ephemeral, revision-bound and `runId`-bound, regenerated on every execution, ignored by Git, and not reusable for another review. External, versioned or immutable release evidence remains Future / Pending for a later phase, together with clean-checkout CI.

The Globals Gate uses the public ESLint `Linter` API as its parser-backed implementation. Its approved implementation and valid execution do not mean that the repository passed: the current result remains blocked by `aerpAuthorize` (`incompatible-public-entrypoint`, CRITICAL), `AERP_AUTHORIZATION_VERSION` (`equivalent-duplicate`, HIGH), `aerpMetadataToRow` (`equivalent-duplicate`, HIGH), and seven productive `eval` uses rejected by the fail-closed dynamic-code policy. Those product corrections are separate work.

The gate's closed IPC v1 interruption contract registers its handlers before emitting `READY` and accepts only validated `INTERRUPT` requests. Controlled Windows SIGINT returns exit 130 with `signal=null`; controlled Windows SIGTERM returns exit 143 with `signal=null`. Evidence persistence, or a sanitized persistence-failure diagnostic, completes before the exit code is set; a persistence failure preserves 130/143. The self-test result is 37 passed with exactly one deliberate POSIX-only skip on Windows. Real POSIX signal coverage remains Future / Pending on a POSIX platform.
