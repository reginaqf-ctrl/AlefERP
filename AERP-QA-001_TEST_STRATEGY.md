...
`npm test` is the implemented canonical Phase 1 command, and `npm run test:all` is its active alias. Except for those two interfaces to the same implemented gate, the controls below remain future expected controls. The executable Phase 1 gate is technically approved, but AERP-QA-001 cannot close until joint code/document review is complete and all applicable gates are implemented, versioned, and passing. Staging remains unauthorized.
...
| Globals                | Future / Pending     | `npm run audit:globals`                                              | Positive bundle                                    | Complete symbol map with zero unresolved collisions                                                              | Architecture owner | Any collision or parse gap                                                                                     | Future release evidence: symbol report                                                                                                                                                |
...
The Full Suite Gate's `.qa-output/test-evidence.json` is local execution evidence only. It is not release evidence. External retention for release remains Future / Pending and MUST be implemented in a later phase.
